...
public class DragingScript : MonoBehaviour {
	...
	public static bool CanMoove = true; //Si le mouvement est permit ou non

	public static int LR = 0;// Left/Right, la direction choisie par l'utilisateur
	...
	void Update () {
		...
		if (LR == 1) { // direction choisie
			//gameObjectToDrag.transform.position = new Vector3(newGoCenter.x, this.transform.position.y, GOcenter.z);
		...
		foreach(Touch touch in Input.touches){
			...
			case TouchPhase.Moved:
				if (dragingeMode && CanMoove){
					...
					if(newGoCenter.x <= -0.5f){ //Le mouvement doit avoir au moins deux pixels de différence avec la position d'orrigine pour éviter les erreures

						LR = 1;
					}

					if(newGoCenter.x >= 0.5f){
						//gameObjectToDrag.transform.position = new Vector3(newGoCenter.x, this.transform.position.y, GOcenter.z);
						LR = 2;
					}
				}
